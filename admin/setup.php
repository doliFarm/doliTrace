...
$item = $formSetup->newItem('DOLITRACE_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('DOLITRACE_MYPARAM5')->setAsYesNo();	 // ->fieldParams['alertifoff'] = 1 or ->fieldParams['alertifon'] = 1;

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('DOLITRACE_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('DOLITRACE_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('DOLITRACE_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('DOLITRACE_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('DOLITRACE_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('DOLITRACE_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('DOLITRACE_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf DOLITRACE_MYPARAM10
$item = $formSetup->newItem('DOLITRACE_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('DOLITRACE_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('DOLITRACE_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('DOLITRACE_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=dolitrace-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
...
$help_url = '';
$title = "DolitraceSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-dolitrace page-admin');
...
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "dolitrace@dolitrace");
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DOLITRACE_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
