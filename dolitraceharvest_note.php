...
/**
 *  \file       dolitraceharvest_note.php
 *  \ingroup    dolitrace
 *  \brief      Tab for notes on DoliTraceHarvest
 */
...
if ($enablepermissioncheck) {
	...
	$permissionnote = $user->hasRight('dolitrace', 'dolitraceharvest', 'write'); // Used by the include of actions_setnotes.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
	$permissionnote = 1;
...
$title = $langs->trans('DoliTraceHarvest').' - '.$langs->trans("Notes");
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("DoliTraceHarvest"), -1, $object->picto);
