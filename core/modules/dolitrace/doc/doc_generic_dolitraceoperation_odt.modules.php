...
/**
 *	\file       htdocs/core/modules/dolitrace/doc/doc_generic_dolitraceoperation_odt.modules.php
 *	\ingroup    dolitrace
 *	\brief      File of class to build ODT documents for dolitraceoperations
 */

dol_include_once('/dolitrace/core/modules/dolitrace/modules_dolitraceoperation.php');
 ...
 */
class doc_generic_dolitraceoperation_odt extends ModelePDFDoliTraceOperation
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DOLITRACE_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$odtPath = trim(getDolGlobalString('DOLITRACE_MYOBJECT_ADDON_PDF_ODT_PATH'));
		...
		$texte .= '<input type="hidden" name="param1" value="DOLITRACE_MYOBJECT_ADDON_PDF_ODT_PATH">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=dolitrace_dolitraceoperation/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=DOLITRACE_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="DOLITRACE_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}
		...
		$texte .= '<input type="hidden" value="DOLITRACE_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *  @param	DoliTraceOperation	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->dolitrace->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new DoliTraceOperation($this->db);
			...
			$dir = $conf->dolitrace->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->dolitrace->dir_temp);
				if (!is_writable($conf->dolitrace->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->dolitrace->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'DOLITRACE_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->dolitrace->dir_temp,
