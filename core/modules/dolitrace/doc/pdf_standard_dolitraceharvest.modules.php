...
/**
 *  \file       core/modules/dolitrace/doc/pdf_standard.modules.php
 *  \ingroup    dolitrace
...
/**
 *	Class to manage PDF template standard_dolitraceharvest
 */
class pdf_standard_dolitraceharvest extends ModelePDFDoliTraceHarvest
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		// ====================================================================
        // [FIX CRITICO] CARICAMENTO TERZO (Azienda Agricola)
        // Se $object->thirdparty è null, il PDF crasha. 
        // Proviamo a caricarlo tramite il Crop Plan collegato.
        // ====================================================================
        if (empty($object->thirdparty)) {
            // Caso 1: Ha un fk_soc diretto (non usato nel tuo caso, ma robusto averlo)
            if (!empty($object->fk_soc) && $object->fk_soc > 0) {
                $object->fetch_thirdparty();
            }
            // Caso 2: Ha un Crop Plan collegato
            elseif (!empty($object->fk_cropplan) && $object->fk_cropplan > 0) {
                dol_include_once('/dolitrace/class/dolitracecropplan.class.php');
                $tmp_cp = new DoliTraceCropplan($this->db);
                if ($tmp_cp->fetch($object->fk_cropplan) > 0) {
                    if ($tmp_cp->fk_soc > 0) {
                        $object->fetch_thirdparty($tmp_cp->fk_soc);
                    }
                }
            }
        }
        
        // Fallback estremo: Se ancora null (es. dati corrotti), creiamo un oggetto vuoto per evitare Fatal Error
        if (empty($object->thirdparty)) {
            $object->thirdparty = new Societe($this->db);
            $object->thirdparty->name = $outputlangs->trans("UnknownThirdParty");
        }
        //
	 ...
	 *
	 *	@param	TCPDF|TCPDI	$pdf     		Object PDF
	 *	@param	DoliTraceHarvest	$object     	Object to show
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		// Show Draft Watermark
		if (getDolGlobalString('DOLITRACE_DRAFT_WATERMARK') && $object->status == $object::STATUS_DRAFT) {
			pdf_watermark($pdf, $outputlangs, $this->page_hauteur, $this->page_largeur, 'mm', dol_escape_htmltag(getDolGlobalString('DOLITRACE_DRAFT_WATERMARK')));
		}
