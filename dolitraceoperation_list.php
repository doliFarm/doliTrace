...
/**
 * \file       dolitraceoperation_list.php
 * \ingroup    dolitrace
 * \brief      List page for dolitraceoperation
 */
...
include_once __DIR__.'/class/dolitraceoperation.class.php';
// [ROBUST] Include linked classes to ensure showOutputField works for Foreign Keys (es. CropPlan)
include_once __DIR__.'/class/dolitracecropplan.class.php'; 

// Load translation files required by the page
// [ROBUST] Added 'dolifarm@dolifarm' to load dictionary labels correctly
$langs->loadLangs(array("dolitrace@dolitrace", "dolifarm@dolifarm", "other"));
...
if ($enablepermissioncheck) {
    ...
    $permissiontodelete = $user->hasRight('dolitrace', 'dolitraceoperation', 'delete');
} else {
    $permissiontoread = 1;
    $permissiontoadd = 1;
...
if (empty($reshook)) {
    ...
    // Mass actions
    $objectclass = 'DoliTraceOperation';
    $objectlabel = 'DoliTraceOperation';
...
$title = $langs->trans("DoliTraceOperations");
$help_url = '';
$morejs = array();
$morecss = array();
...
foreach ($search as $key => $val) {
    if (array_key_exists($key, $object->fields)) {
        ...
        if ((strpos($field_spec['type'], 'integer:') === 0) || (strpos($field_spec['type'], 'sellist:') === 0) || !empty($field_spec['arrayofkeyval'])) {
            ...
            $mode_search = 2; // Exact match for IDs or Keys
        }
...
if (!getDolGlobalInt('MAIN_DISABLE_FULL_SCANLIST')) {
    ...
    if (($page * $limit) > (int) $nbtotalofrecords) {   // if total resultset is smaller than the paging size
        $page = 0;
        $offset = 0;
    }
    $db->free($resql);
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: ".dol_buildpath('/dolitrace/dolitraceoperation_card.php', 1).'?id='.((int) $id));
    exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-dolitrace page-list bodyforlist');
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolitrace/dolitraceoperation_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action
$topicmail = "SendDoliTraceOperationRef";
$modelmail = "dolitraceoperation";
$objecttmp = new DoliTraceOperation($db);
...
foreach ($object->fields as $key => $val) {
    ...
    if (!empty($arrayfields['t.'.$key]['checked'])) {
        ...
        if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) {
            ...
            // [ROBUST] Use showInputField to handle lookup fields (CropPlan, etc.) correctly in search
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban' || $mode == 'kanbangroupby') {
        ...
        foreach ($object->fields as $key => $val) {
            ...
            if (in_array($val['type'], array('double(24,8)', 'double(6,3)', 'integer', 'real', 'price')) && !in_array($key, array('id', 'rowid', 'ref', 'status')) && $val['label'] != 'TechnicalID' && empty($val['arrayofkeyval'])) {
                $cssforfield .= ($cssforfield ? ' ' : '').'right';
            }
            ...
            if (!empty($arrayfields['t.'.$key]['checked'])) {
                ...
                if ($key == 'status') {
                    ...
                    // [ROBUST] showOutputField will handle the FK and Dictionary translation automatically
                    // because we defined the class fields correctly.
