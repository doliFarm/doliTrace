...
/**
 *   	\file       dolitracecropplan_list.php
 *		\ingroup    dolitrace
 *		\brief      List page for dolitracecropplan
 */
...
include_once __DIR__.'/class/dolitracecropplan.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('dolitrace', 'dolitracecropplan', 'write');
	$permissiontodelete = $user->hasRight('dolitrace', 'dolitracecropplan', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("dolitrace")) {
	accessforbidden('Module dolitrace not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DoliTraceCropplan';
	$objectlabel = 'DoliTraceCropplan';
...
$title = $langs->trans("DoliTraceCropplans");
//$help_url = "EN:Module_DoliTraceCropplan|FR:Module_DoliTraceCropplan_FR|ES:Módulo_DoliTraceCropplan";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/dolitrace/dolitracecropplan_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-dolitrace page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolitrace/dolitracecropplan_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDoliTraceCropplanRef";
$modelmail = "dolitracecropplan";
$objecttmp = new DoliTraceCropplan($db);
