...
/**
 *  \file       dolitracecropplan_document.php
 *  \ingroup    dolitrace
 *  \brief      Tab for documents linked to DoliTraceCropplan
 */
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->dolitrace->multidir_output[$object->entity ? $object->entity : $conf->entity]."/dolitracecropplan/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('dolitrace', 'dolitracecropplan', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("DoliTraceCropplan")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("DoliTraceCropplan"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='dolitracecropplan/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'dolitracecropplan/'.dol_sanitizeFileName($object->ref).'/';
