...
/**
 *  \file       dolitraceharvest_document.php
 *  \ingroup    dolitrace
 *  \brief      Tab for documents linked to DoliTraceHarvest
 */
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->dolitrace->multidir_output[$object->entity ? $object->entity : $conf->entity]."/dolitraceharvest/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('dolitrace', 'dolitraceharvest', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("DoliTraceHarvest")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("DoliTraceHarvest"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='dolitraceharvest/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'dolitraceharvest/'.dol_sanitizeFileName($object->ref).'/';
